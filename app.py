...
from evaluation.semantic_similarity import compute_semantic_similarity, compute_section_relevance

# Visualization imports
from visualization.diff_viewer import compute_line_diff, calculate_diff_stats

# Semantic Engine Check
from models.embedding_model import is_semantic_available, get_load_error
...
def run_optimization():
    ...
    with st.spinner("Running full pipeline..."):
        ...
        semantic_score = compute_semantic_similarity(raw_prompt, repaired_prompt)
        
        # Update Prompt IR sections with relevance scores
        task_text = prompt_ir.get("task", {}).get("text", "")
        for section in prompt_ir.get("sections", []):
            section["relevance_score"] = compute_section_relevance(section.get("content", ""), task_text)
        ...
        # Final Score (Automatically handles semantic fallback)
...
with st.sidebar:
    ...
    st.markdown("**Phase 2: Semantic Intelligence**")
    st.divider()
    if is_semantic_available():
        st.success("✅ Semantic Engine Active\n(all-MiniLM-L6-v2)")
    else:
        st.warning("⚠️ Semantic Engine Offline\nUsing rule-based metrics only.")
        err = get_load_error()
        if err:
            st.caption(f"Error: {err[:50]}...")
...
if st.session_state.optimization_complete:
    ...
    with tab2:
        ...
        m6.metric("Semantic Similarity", f"{m['semantic_similarity']*100:.1f}%" if m['semantic_similarity'] >= 0 else "N/A")
    ...
    with tab3:
        ...
        # Show sections with their new relevance scores
        st.markdown("### Sections with Relevance Scores")
        for sec in ir.get("sections", []):
            score = sec.get("relevance_score", -1)
            score_str = f"{score*100:.1f}%" if score >= 0 else "N/A"
            st.markdown(f"- **{sec['name']}** (Relevance: {score_str}): `{sec['content'][:50]}...`")
            
        st.divider()
        st.markdown("### Full IR JSON")
        st.json(ir)
